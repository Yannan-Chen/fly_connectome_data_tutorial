...
# Run UMAP
set.seed(42)
umap_result <- umap(influence_matrix_umap, n_neighbors = 15, min_dist = 0.1)
...
Now let's make a static facet plot, where we visualise influence from the top 9 source types!

```{r abdominal_facet_umap, fig.width=14, fig.height=14}
# Work out the top 9 source types by total influence
...
source_influence_summary <- abdominal_influence_all %>%
  filter(target_sub_class %in% top9_sources) %>%
  group_by(source_type, target_sub_class) %>%
  summarise(
...
# We need to create a matrix where each row is an abdominal type and columns are the top9 sources
...
Try modifying this analysis to explore different questions:

```{r exercise, eval=FALSE}
# Exercise 1: Analyse influence from descending neurons to motor neurons
descending_neurons <- meta %>%
  filter(flow == "descending") %>%
  pull(!!sym(dataset_id))

motor_neurons <- meta %>%
  filter(super_class == "motor_neuron")

# Exercise 2: Compare influence patterns across different datasets
# Try with FAFB, MANC, or maleCNS by changing the dataset parameter

# Exercise 3: Analyse influence within a specific neuropil
# Filter neurons by region column before calculation

# Exercise 4: Compare influence vs. direct connectivity
# Calculate both and see how they differ for the same neuron pairs
```
