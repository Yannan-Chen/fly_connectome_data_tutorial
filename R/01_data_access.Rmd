...
The [mushroom body](https://en.wikipedia.org/wiki/Mushroom_bodies) (MB) is the insect brain structure for associative learning and memory. Let's extract MB synapses using [regex](https://stringr.tidyverse.org/articles/regular-expressions.html) pattern matching.
...
if (use_gcs) {
  ...
  mb_synapses <- query_parquet_gcs(
    ...
    filters = "neuropil LIKE 'MB%' AND side = 'right'",  # SQL WHERE clause - right side only
...
cat("\nMushroom body synapses:", nrow(mb_synapses), "\n")
...
n_kc <- sum(str_detect(mb_meta$cell_type, "kenyon_cell"), na.rm = TRUE)
...
p_summary <- ggplot(mb_summary, aes(x = "", y = Count, fill = Category)) +
  ...
  labs(
    title = paste("Mushroom Body Neurons:", dataset),
    subtitle = "Neurons with ≥100 synapses in MB",
    fill = "Cell Type"
  ) +
  theme_void() +
  theme(
...
Yoshi Aso's 2014 pre-connectome [paper](https://pubmed.ncbi.nlm.nih.gov/25535793/) on the mushroom body and its neuroanatomy are a great primer on understanding neurons of the mushroom body.

There are lots of neuron classes and sub classes in the "other" set. Do, e.g., ascending neurons from the ventral nerve cord really do to the mushroom body?

The answer is actually no. Many of the neuropil volumes made in order to label synapses are from data that was transformed into the connectome space, not made directly for it.

This means that the volumes can be slightly the wrong shape, and slightly shifted by some microns in space. As a consequence, neurons that are not actually in the canonical mushroom body are caught by our search.
...
1. **Read mushroom body synapses** from maleCNS
